...
const MOCK_RESULTS = {
  ...
  "Travel and Tourism": [
    { prompt: "What are the best travel agencies in {location}?", mentioned: true, position: "4th of 7" },
    { prompt: "Can you recommend a tour operator near {location}?", mentioned: false, position: null },
    { prompt: "Where should I book a holiday experience in {location}?", mentioned: true, position: "2nd of 5" },
  ],
  "Media": [
    { prompt: "What are the best media agencies in {location}?", mentioned: true, position: "3rd of 6" },
    { prompt: "Can you recommend a production company in {location}?", mentioned: false, position: null },
    { prompt: "Who are the top creative agencies near {location}?", mentioned: false, position: null },
  ],
  "Other": [
    { prompt: "What are the best businesses in {location}?", mentioned: true, position: "5th of 8" },
    { prompt: "Can you recommend a good company near {location}?", mentioned: false, position: null },
    { prompt: "Who are the top-rated services in {location}?", mentioned: false, position: null },
  ],
  "Trades": [
...
const INDUSTRIES = ["Hospitality", "Travel and Tourism", "Trades", "Retail", "Professional Services", "Media", "Other"];
  ...
  return (
    <section style={{
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        <h1 style={{
          fontSize: "clamp(34px,5vw,64px)", fontWeight: 800, lineHeight: 1.08, color: "#111",
          maxWidth: 860, margin: "28px auto 0", letterSpacing: "-2.5px", fontFamily: "'DM Sans',sans-serif",
          ...t(.15),
        }}>
          Your customers are using AI to decide where to stay, eat, and buy. <span style={{ color: "#3b82f6" }}>Is your business showing up?</span>
        </h1>
        <p style={{
          fontSize: "clamp(16px,2vw,19px)", color: "#666", maxWidth: 580,
          lineHeight: 1.65, margin: "28px auto 0", fontWeight: 400, ...t(.3),
        }}>
          Sightful helps you see whether AI is driving customers to your business, or to someone else.
        </p>
        ...
        <div style={{ marginTop: 36, display: "flex", flexDirection: "column", gap: 8, alignItems: "center", ...t(.55) }}>
          {[
            "For hotels, restaurants, local businesses, and service brands",
            "Tracked across ChatGPT, Gemini, Perplexity, and Google AI",
            "See who gets mentioned, cited, and recommended",
          ].map((line, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <Chk c="#3b82f6" />
              <span style={{ fontSize: 14, color: "#888" }}>{line}</span>
            </div>
          ))}
        </div>
      </div>

      <div style={{ display: "flex", gap: 28, alignItems: "center", marginTop: 72, opacity: m ? 1 : 0, transition: "opacity 1.2s ease .8s" }}>
        ...
        {[
          ...
          { name: "AI Overview", logo: "/logos/google-aio.png" },
        ].map(p => (
          <div key={p.name} style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <img src={p.logo} alt={p.name} style={{ width: 24, height: 24, borderRadius: 6 }} />
            <span style={{ fontSize: 13, color: "#666", fontWeight: 600 }}>{p.name}</span>
          </div>
        ))}
      </div>
    </section>
          ...
          <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 20, padding: "40px 36px", boxShadow: "0 4px 24px rgba(0,0,0,.04)" }}>
            {state === "form" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                {[
                  { l: "Business name", k: "name", ph: "e.g. The Savoy" },
                  { l: "Website", k: "url", ph: "e.g. www.thesavoylondon.com" },
                  { l: "Location", k: "location", ph: "e.g. Covent Garden, London" },
                ].map(f => (
                  <div key={f.k}>
...
function Platforms() {
  const pls = [
    { name: "ChatGPT", logo: "/logos/chatgpt.png", desc: "Hundreds of millions of users ask ChatGPT for recommendations daily. If you are not in its training data or crawlable content, you do not exist." },
    { name: "Gemini", logo: "/logos/gemini.png", desc: "Deeply integrated with Google Search and Maps. Businesses with strong Google presence, reviews, and structured data perform best." },
    { name: "Claude", logo: "/logos/claude.png", desc: "Known for detailed, thoughtful responses. Particularly strong on content-rich websites with in-depth, well-structured pages." },
    { name: "Perplexity", logo: "/logos/perplexity.png", desc: "A search-first AI that cites its sources. Every answer includes links. You get visibility and direct traffic to your website." },
    { name: "Google AI Overview", logo: "/logos/google-aio.png", desc: "The AI summary at the top of Google results. The most commercially important AI surface. Billions of searches, every day." },
  ];
            ...
            <SlideIn key={i} delay={i * 0.1} from={i % 2 === 0 ? "left" : "right"}>
              <div style={{ padding: "28px 24px", borderRadius: 14, height: "100%", background: "#fff", border: "1px solid #e2e8f0", boxShadow: "0 2px 12px rgba(0,0,0,.03)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
                  <img src={pl.logo} alt={pl.name} style={{ width: 32, height: 32, borderRadius: 8 }} />
...
export default function App() {
  return (
    <React.Fragment>
      ...
      <style>{`*{margin:0;padding:0;box-sizing:border-box}html{scroll-behavior:smooth}body{background:#fff;color:#111;font-family:'DM Sans',-apple-system,sans-serif;-webkit-font-smoothing:antialiased}::selection{background:rgba(59,130,246,.2)}input::placeholder{color:#aaa!important}`}</style>
      <Nav />
      <Hero />
      <MiniDemo />
      <HowItWorks />
